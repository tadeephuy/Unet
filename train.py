...
def train(model, criterion, optimizer, scheduler, n_epochs, batch_size, save_interval, config_path):
    ...
    csv_path_train = "D:/Dataset/ICDAR2013/train.csv"
    csv_path_val = "D:/Dataset/ICDAR2013/test.csv"
    root_input_dir = "D:/Dataset/ICDAR2013/"
    # input_ext = 'jpg'
    ...
    icdaar2013_dataset_train = SegmentationDataset(
        ...
        root_input_dir=root_input_dir + 'Train_data', input_ext='jpg',
        root_label_dir=root_input_dir + 'Train_GT', label_ext='bmp',
        label_prefix='', label_suffix='_GT',
        transform=transform
    )
    icdaar2013_dataset_val = SegmentationDataset(
        csv_path=csv_path_val,
        root_input_dir=root_input_dir + 'Test_data', input_ext='jpg',
        root_label_dir=root_input_dir + 'Test_GT', label_ext='png',
        label_prefix='gt_', label_suffix='',
        transform=transform
    )
    ...
    for epoch_idx in range(n_epochs):
        ...
        print('Epoch {}/{} - Best loss: {:.4f} - Dur: {:.0f}m {:.0f}s - Train: Loss: {:.4f} Acc: {:.2f}, Val: Loss: {:.4f} Acc: {:.2f}'.format(epoch_idx,
                                                                                                                                            n_epochs - 1,
                                                                                                                                            best_loss,
                                                                                                                                            time_elapsed // 60,
                                                                                                                                            time_elapsed % 60,
...
unet = Unet(config_path=model_config_path, is_training=True, weights_path="D:/Projects/DeepLearning/Unet/weights/Best-Epoch28-Loss0.1910.pt").model
unet.to(device)
