...
class Unet():
    ...
    def process(self, x):
        ...
        x = self.preprocess(x) # (c, h, w)
        # batch 1
        x = x.unsqueeze(0) # (1, c, h, w)
        output = self.model(x)
        output = output.squeeze(0).detach().numpy() # (c, h, w)
    ...
    def postprocess(self, x, resize=False):
        ...
        x = np.vectorize(lambda value: 0 if value == 0 else 255)(x)
        return x
...
class UnetModel(nn.Module):
    ...
    def down_sampling_block(self, in_channels, out_channels, kernel_size=(3, 3)):
        block = nn.Sequential(
    ...
    def up_sampling_block(self, in_channels, out_channels, kernel_size=(3, 3)):
    ...
    def output_block(self, in_channels, out_channels, kernel_size=(3, 3)):
        mid_channels = in_channels // 2
